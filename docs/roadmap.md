...
- `pnpm v1:release-audit` を追加し、v1リリース条件を `release.docs` / `model.default` / `eval.readiness` / `smoke.current_environment` / `export.qa` / `g4.human_review` / `smoke.fresh_environment` に分けて監査できるようにした。直近runは `INCOMPLETE` で、既存環境の機械証跡はPASS、人間G4と新規環境スモーク証跡は未達。
