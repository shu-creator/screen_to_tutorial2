...
- `pnpm v1:smoke -- --video <sample.mp4> ...` を追加し、`setup:check`、`pipeline:generate`、`project:export`、`edit:smoke` を1コマンドで実行して `v1_smoke_summary.json` に集約できるようにした。既存ローカル環境で `eval/dataset/synth-login-click-01/video.mp4` を使った直近runは project 31、`pass=true`、steps SHA-256 `e68f08df86f6d7747251e5ad4f84f619f83514da15a6dfd450109822704d9d58`、`step_count=3`、`needs_review_count=3`、`fallback_reason_count=0`。
