...
## LLMモデルをgemini-3-flashに変更

- [x] server/_core/llm.tsのモデル名を変更
- [x] 動作確認（ステップ生成が正常に動作するか）
- [x] GitHubに反映（ローカルにコミット済み）

## 動画アップロード時のForbiddenエラーを修正

- [x] エラーが発生している箇所を特定（videoProcessor.tsのdownloadFile関数）
- [x] S3ダウンロード権限の問題を調査（署名付きURLが必要）
- [x] 修正を実装（Manusストレージプロキシ経由でダウンロード）
- [x] 動作確認（正常に動作）
